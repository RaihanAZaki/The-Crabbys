<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
  <section class="py-10 bg-gray-50 font-poppins dark:bg-gray-800 rounded-2xl">
      ...
      <div class="flex flex-wrap mb-24 -mx-3">
        <div class="w-full pr-2 lg:w-1/4 lg:block">
          <div class="p-4 mb-5 bg-white border border-gray-200 rounded-lg dark:border-gray-900 dark:bg-gray-900">
            ...
            <div class="w-16 pb-2 mb-6 border-b border-[#D97707] dark:border-gray-400"></div>
            <ul>
              @foreach ($categories as $category)
              <li class="mb-4" wire:key="{{ $category->id }}">
                <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                  <input type="checkbox" wire:model.live='selected_categories' id="{{$category->slug}}" value="{{$category->id}}" class="w-4 h-4 mr-2 rounded-md">
          ...
          <div class="p-4 mb-5 bg-white border rounded-2xl border-gray-200 dark:bg-gray-900 dark:border-gray-900">
            <h2 class="text-2xl font-bold dark:text-gray-400">Product Status</h2>
            <div class="w-16 pb-2 mb-6 border-b border-[#D97707] dark:border-gray-400"></div>
            <ul>
              <li class="mb-4">
                <label for="" class="flex items-center dark:text-gray-300">
                  <input type="checkbox" wire:model.live='is_popular' class="w-4 h-4 mr-2 rounded-md">
                  <span class="text-lg dark:text-gray-400">Popular</span>
                </label>
              </li>
            </ul>
          </div>

          <div class="p-4 mb-5 rounded-2xl bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
            <h2 class="text-2xl font-bold dark:text-gray-400">Price</h2>
            <div class="w-16 pb-2 mb-6 border-b border-#D97707] dark:border-gray-400"></div>
            <div>
              <div class="font-semibold text-sm">{{Number::currency($price_range, 'IDR', 'Rp')}}</div>
              <input type="range" wire:model.live='price_range' class="slider" max="500000" value="100000" step="1000">
              <div class="flex justify-between ">
                <span class="inline-block text-md font-bold text-[#D97707] ">{{Number::currency(1000, 'IDR', 'Rp')}}</span>
                <span class="inline-block text-md font-bold text-[#D97707] ">{{Number::currency(500000, 'IDR', 'Rp')}}</span>
              </div>
            </div>
          </div>
        </div>
        <div class="w-full px-3 lg:w-3/4">
          <div class="px-3 mb-4">
            <div class="items-center justify-between hidden px-3 py-2 rounded-lg bg-gray-100 md:flex dark:bg-gray-900 ">
              <div class="flex items-center justify-between">
                <select wire:model.live="sort" class="block w-40 rounded-md text-base bg-gray-100 cursor-pointer dark:text-gray-400 dark:bg-gray-900">
          ...
          <div class="flex flex-wrap items-center ">
            @foreach ($products as $product)
              <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3 rounded-lg" wire:key='{{$product->id}}'>
                <div class="border border-gray-300 rounded-lg dark:border-gray-700">
                  ...
                  <div class="p-3 ">
                    ...
                    <p class="text-lg ">
                      <span class="text-[#D97707] dark:text-green-600">{{ Number::currency($product->price, 'IDR', 'Rp') }}</span>
                    </p>
                  </div>
