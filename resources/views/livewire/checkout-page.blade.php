				...
				<div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
					...
					<div class="mb-6">
						...
						<div class="mt-4">
							...
							<select wire:model='table_id' class="w-full rounded-lg border py-2 px-3 dark:bg-gray-700 dark:text-white dark:border-none @error('table') border-red-500 @enderror" id="table_id">
								@if($table && $table->count())
									@foreach ($table as $item)
										<option value="{{ $item->id }}">{{ $item->name }}</option>
									@endforeach
								@else
									<option disabled>Tidak ada tabel tersedia</option>
								@endif
							</select>
							@error('meja')
								<div class="text-red-500 text-xs">{{$message}}</div>
			...
			<div class="md:col-span-12 lg:col-span-4 col-span-12">
				<div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
					...
					<div class="flex justify-between mb-2 font-bold">
						...
						<span>
							{{Number::currency($grand_total, 'IDR', 'RP')}}
						</span>
					</div>
					<div class="flex justify-between mb-2 font-bold">
						<span>
						...
						<li class="py-3 sm:py-4" wire:key='{{$item['product_id']}}'>
							<div class="flex items-center">
								...
								<div class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
									{{Number::currency($item['total_amount'], 'IDR', 'RP')}}
								</div>
							</div>
						</li>
						@endforeach
