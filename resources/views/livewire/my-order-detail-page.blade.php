<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
  ...
  <div class="flex flex-col md:flex-row gap-4 mt-4">
    <div class="md:w-3/4">
        ...
        <table class="w-full">
          ...
          <tbody>
            ...
            <tr wire:key="{{$item->id}}">
              ...
              <td class="py-4">{{Number::currency($item->unit_amount, 'IDR', 'RP')}}</td>
              <td class="py-4">
                <span class="text-center w-8">{{$item->quantity}}</span>
              </td>
              <td class="py-4">{{Number::currency($item->total_amount, 'IDR', 'RP')}}</td>
            </tr>
            @endforeach
          </tbody>
        </table>
      ...
      <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
        <h1 class="font-3xl font-bold text-slate-500 mb-3">Address</h1>
        <div class="flex justify-between items-center">
          <div>
            <p class="font-semibold">Table:</p>
            <p>{{$address->meja->name}}</p>
          </div>
          <div>
    ...
    <div class="md:w-1/4">
      <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="flex justify-between mb-2">
          <span>Subtotal</span>
          <span>{{Number::currency($item->order->grand_total, 'IDR', 'RP')}}</span>
        </div>
        <div class="flex justify-between mb-2">
          <span>Taxes</span>
          <span>{{Number::currency(0, 'IDR', 'RP')}}</span>
        </div>
        <hr class="my-2">
        <div class="flex justify-between mb-2">
          <span class="font-semibold">Grand Total</span>
          <span class="font-semibold">{{Number::currency($item->order->grand_total, 'IDR', 'RP')}}</span>
        </div>
